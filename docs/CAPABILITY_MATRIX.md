...
| Windows file association | Read-only HKCU registry snapshot, status model, Settings summary detection, current-exe expected path detection, egui status panel, and pure install/repair write plan exist | Add registry writer plus install/repair/uninstall UI actions |
| Windows thumbnails | Read-only registration status model, Settings summary detection, co-located DLL expected path detection, egui status panel, and pure registration write plan exist | Build Shell thumbnail provider DLL and register/repair actions |
