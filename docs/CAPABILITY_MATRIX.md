...
| Archived video segments | Segment path listing exists in `libs/silica` with numeric ordering tests; `libs/silica` can extract ordered segment bytes; `silicate` can stage segment/temp-list files through an injected writer, build an ffmpeg concat-list merge command plan, run it through an injected ffmpeg runner, and gate export on detected ffmpeg tool status before staging | Add timing metadata, wire user-facing output selection/export flow, export full/30s |
