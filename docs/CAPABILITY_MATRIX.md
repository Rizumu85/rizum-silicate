...
| Windows file association | Read-only HKCU registry snapshot, status model, Settings summary detection, current-exe expected path detection, egui status panel, registry writer, and install/repair action exist | Add uninstall action and shell refresh/cache invalidation |
| Windows thumbnails | Read-only registration status model, Settings summary detection, co-located DLL expected path detection, egui status panel, registry writer, and install/repair registration action exist | Build Shell thumbnail provider DLL, uninstall action, and shell refresh/cache invalidation |
