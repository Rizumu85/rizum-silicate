...
| Windows thumbnails | Read-only registration status model, Settings summary detection, co-located DLL expected path detection, egui status panel, registry writer, install/repair registration action, uninstall action, Explorer association-change notification, explicit Restart Explorer action, shared PNG/RGBA platform thumbnail loader, and Windows provider crate with `IThumbnailProvider`/`IInitializeWithFile` object tests exist | Implement Shell class factory/DLL exports, validate Explorer loading, and add deeper thumbnail cache handling |
