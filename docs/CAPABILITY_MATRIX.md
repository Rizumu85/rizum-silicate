...
| Archived video segments | Segment path listing exists in `libs/silica` with numeric ordering tests; `libs/silica` can extract ordered segment bytes; `silicate` can stage segment/temp-list files through an injected writer, build an ffmpeg concat-list merge command plan, and run it through an injected ffmpeg runner | Add timing metadata, wire user-facing export flow, export full/30s |
| Batch export | Not implemented | Folder/multi-file queue, progress, retry |
| ffmpeg sidecar | Pure ffmpeg tool detection exists for bundled/system/missing status, Settings reports Video Tools, command construction uses an injected executable path, and command execution uses an injected runner boundary | Bundle LGPL build and wire export jobs to the detected tool |
