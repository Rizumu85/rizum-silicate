...
| Windows thumbnails | Read-only registration status model, Settings summary detection, co-located DLL expected path detection, egui status panel, registry writer, install/repair registration action, uninstall action, Explorer association-change notification, explicit Restart Explorer action, shared PNG/RGBA platform thumbnail loader, and Windows provider crate with path/in-memory archive bitmap loading, `IThumbnailProvider`/`IInitializeWithFile`, `IClassFactory`, `DllGetClassObject`, and `DllCanUnloadNow` tests exist | Validate Explorer loading through the registered DLL and add deeper thumbnail cache handling |
