...
| Windows file association | Read-only HKCU registry snapshot, status model, Settings summary detection, current-exe expected path detection, and egui status panel exist | Add install/repair/uninstall actions |
| Windows thumbnails | Read-only registration status model, Settings summary detection, co-located DLL expected path detection, and egui status panel exist | Build Shell thumbnail provider DLL and register/repair actions |
